...
`SecurityUtil.authUserId()` это идентификатор `id` авторизированного юзера. Мы можем, например, получить объект авторизированного юзера, у которого `User.id==SecurityUtil.authUserId()`
Когда вы логинитесь в свое почтовое приложение, оно отдает вам именно ваши письма на основе вашего `id`, который она определяет и запоминает во время логина.  
У нас пока нет авторизации, это `id` задается константой (хардкодится). Но когда мы сделаем настоящую авторизацию, все будет работать для любого залогиненного пользователя.
...
Настоящая авторизация будет через Spring Security позже.
---------------------
